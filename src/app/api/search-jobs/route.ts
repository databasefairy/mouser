...
import { runGeminiSearchOnly, runGeminiFastMode } from "@/lib/search-jobs/gemini-agent";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fast mode: AI-generated suggestions with lightweight URL validation
    if (input.fast_mode) {
      try {
        const startTime = Date.now();

        // Enhanced URL validation - GET request with content checks for Workday and other ATS
        async function checkUrlExists(url: string): Promise<boolean> {
          ...
          try {
            // Use GET (not HEAD) to get actual page content - more reliable
            const response = await axios.get(url, {
              timeout: 8000,
              maxRedirects: 5,
            ...
            // Workday: Check if redirected to job search page (means job doesn't exist)
            if (isWorkday) {
              // If URL has /job/ but final URL doesn't, it redirected away from the job
              if (url.includes("/job/") && !finalUrl.includes("/job/")) {
                console.log(`[fast-mode] Workday redirect away from job: ${url} -> ${finalUrl}`);
                return false;
              }

              // Check for Workday-specific "job not found" patterns
              if (
                htmlContent.includes("job search") && htmlContent.includes("<title>") &&
                (htmlContent.match(/<title[^>]*>.*?job search.*?<\/title>/i) ||
                  htmlContent.match(/<title[^>]*>.*?search jobs.*?<\/title>/i) ||
                  htmlContent.match(/<title[^>]*>.*?careers.*?<\/title>/i) && !htmlContent.includes("job description"))
              ) {
                console.log(`[fast-mode] Workday job search page detected: ${url}`);
                return false;
              }
            }

            // Check for common "job not found" indicators across all platforms
            const deadKeywords = [
              "job no longer available",
              "position has been filled",
              "job is closed",
              ...
              "job you are looking for is not",
              "job you are looking for is no longer",
              "this job is no longer",
            ];

            const hasDead = deadKeywords.some(kw => htmlContent.includes(kw));
            if (hasDead) {
              console.log(`[fast-mode] Dead job keywords found: ${url}`);
              return false;
            }

            return true;
          } catch (err) {
            console.log(`[fast-mode] URL check failed: ${url} - ${err instanceof Error ? err.message : "error"}`);
            return false;
          }
        }
        ...
        let iteration = 0;
        const MAX_ITERATIONS = 10; // Higher limit to ensure we get requested count

        console.log(`[search-jobs] Fast mode: finding ${input.top_n} jobs with valid URLs`);

        // Keep requesting until we have enough valid jobs
        while (validJobs.length < input.top_n && iteration < MAX_ITERATIONS) {
          iteration++;
          const stillNeeded = input.top_n - validJobs.length;
          console.log(`[search-jobs] Fast mode iteration ${iteration}: have ${validJobs.length}/${input.top_n} valid jobs, need ${stillNeeded} more`);

          // Request more than we need to account for invalid URLs (2x the remaining needed)
          const requestCount = Math.max(stillNeeded * 2, 15);

          const response = await runGeminiFastMode(geminiKey, {
            top_n: requestCount,
          ...
          // Parse JSON response
          let rawJson = response.outputText.trim();
          ...
          try {
            ...
            console.error("[search-jobs] Fast mode parse error:", parseErr);
            continue;
          }

          console.log(`[search-jobs] Fast mode: checking ${jobs.length} URLs for validity`);

          // Check URLs in parallel
          const urlChecks = await Promise.all(
            jobs.map(async (job) => {
              const url = job.direct_apply_link;
              ...
              if (!exists) {
                console.log(`[search-jobs] Fast mode: invalid URL ${url}`);
                return null;
              }
          ...
          // Add valid jobs
          const newValidJobs = urlChecks.filter((j): j is JobResult => j !== null);
          validJobs.push(...newValidJobs);

          console.log(`[search-jobs] Fast mode: found ${newValidJobs.length} new valid jobs (total: ${validJobs.length})`);

          // If we have enough, stop
          if (validJobs.length >= input.top_n) break;
        }

        // Trim to requested count
        const finalJobs = validJobs.slice(0, input.top_n);

        const duration = Date.now() - startTime;
        console.log(`[search-jobs] Fast mode completed in ${duration}ms: ${finalJobs.length} jobs with valid URLs (${iteration} iterations)`);

        return NextResponse.json({
          query_used: { ...input, resume_text: undefined, resume_provided: !!input.resume_text?.trim(), fast_mode: true },
          results: finalJobs,
          excluded_counts: { not_active: seenUrls.size - finalJobs.length },
          missing_info: finalJobs.length < input.top_n ? [`Only found ${finalJobs.length} jobs with valid URLs (requested ${input.top_n})`] : [],
          fast_mode: true,
          duration_ms: duration,
        });
      } catch (err) {
        console.error("[search-jobs] Fast mode error:", err);
        return NextResponse.json(
          { error: err instanceof Error ? err.message : "Fast mode failed" },
          { status: 500 }
        );
      }
    }
